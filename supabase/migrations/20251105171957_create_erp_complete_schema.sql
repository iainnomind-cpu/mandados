/*
  ...
  - `profiles` - User profiles with role-based access
    ...
    - `full_name` (text)
    - `role` (enum: admin, operator, dispatcher, finance, driver)
    - `phone` (text)
    - `avatar_url` (text)
    - `active` (boolean)
  ...
  - `chat_conversations` - Chat session tracking
    ...
    - `channel` (text: whatsapp, web, phone)
  ...
  - `chat_messages` - Conversation messages
    ...
    - `sender_type` (enum: customer, bot, operator)
    - `message` (text)
    - `metadata` (jsonb)
    - `created_at` (timestamptz)
  ...
  - `orders` - Centralized order registry
    ...
    - `customer_id` (uuid, FK to customers)
    - `conversation_id` (uuid, FK to chat_conversations, nullable)
    - `order_type` (enum: mandadito, restaurant, express)
    - `source` (enum: chatbot, phone, web, restaurant)
    ...
    - `payment_status` (enum: pending, paid, cod)
  ...
  - `assignments` - Driver-order assignments
    ...
    - `status` (enum: assigned, accepted, rejected, in_progress, completed, cancelled)
  ...
  - `driver_advances` - Driver cash advances
    ...
    - `driver_id` (uuid, FK to drivers)
    - `amount` (numeric)
    - `type` (enum: advance, loan, bonus)
    - `status` (enum: pending, approved, disbursed, deducted)
  ...
  - `transactions` - Financial transaction registry
    ...
    - `transaction_type` (enum: collection, commission, advance_deduction, fee, refund)
    - `amount` (numeric)
    - `payment_method` (enum: cash, card, transfer)
  ...
  - `reconciliations` - Daily reconciliation records
    ...
    - `reconciled_by` (uuid, FK to profiles, nullable)
    - `notes` (text)
    - `created_at` (timestamptz)
